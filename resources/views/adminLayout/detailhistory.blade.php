...
@php
    $selisihHari = \Carbon\Carbon::parse($item->tanggal_pengambilan)->diffInDays(\Carbon\Carbon::now());
@endphp
        ...
        <tbody class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            ...
            @if($item->tanggal_pengembalian === null)
                <tr
                    ...
                    <td class="px-6 py-3">
                        <!-- {{ number_format(\Carbon\Carbon::parse($item->tanggal_pengambilan)->diffInDays(\Carbon\Carbon::now()), 0, '.', ',') }} -->
                        @if($selisihHari < 30)
